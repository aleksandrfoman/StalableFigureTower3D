...
public class ParticlesSpawn : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        for (int i = 0; i < 4; i++)
        {
           particles.Add(Instantiate(particle, holder.transform));
        }
    }
        ...
        foreach (var item in collision.contacts)
        {
            if (collision.collider.GetComponent<Renderer>())
            {
                if (particles.Count != 0)
                {
                    ...
                    parts.GetComponent<ParticleSystemRenderer>().material.SetColor("_Color", collision.collider.GetComponent<Renderer>().material.GetColor("_Color") / 1.1f);
